...
/// Synthetic callback workload; excludes native presentation, network and image decoding.
pub fn benchmark() {
	...
	let mut frame = || {
		ctx.run_ui(
			eframe::egui::RawInput {
				screen_rect: Some(eframe::egui::Rect::from_min_size(
					eframe::egui::Pos2::ZERO,
					eframe::egui::vec2(1120.0, 760.0),
				)),
				..Default::default()
			},
			|ui| {
				assert!(view.show(ui, &mut state).is_empty());
			},
		)
	...
	println!("Component callback samples (200 frames, ms): {samples:?}");
	samples.sort_by(f64::total_cmp);
	println!("Median callback: {:.3} ms/frame", samples[2] / 200.0);
}
