...
pub struct Glyph {
    ...
    /// Resolved direction; the glyph array remains in source order.
    #[cfg_attr(feature = "serde", serde(default))]
    pub is_rtl: bool,

    /// A row break may precede this scalar without splitting a shaped cluster.
    #[cfg_attr(feature = "serde", serde(default))]
    pub(crate) cluster_start: bool,
    ...
    #[cfg_attr(feature = "serde", serde(default))]
    pub(crate) extra_uv_rects: Option<Arc<[UvRect]>>,
...
impl Row {
    ...
    pub fn char_at(&self, desired_x: f32) -> CharIndex {
        if self.glyphs.iter().any(|g| g.is_rtl) {
            return self.nearest_caret(desired_x).0;
        }
    ...
    pub fn x_offset(&self, column: CharIndex) -> f32 {
        if let Some(glyph) = self.glyphs.get(column.0) {
            ...
            .glyphs
            .last()
            .filter(|_| self.glyphs.iter().any(|g| g.is_rtl))
        {
            if glyph.is_rtl {
                glyph.pos.x
            } else {
    ...
    /// On bidi rows, `prefer_next_row` also selects the next character's leading
    /// edge; false selects the previous character's trailing edge. At a soft
    /// wrap these affinities continue to select the next/previous row as usual.
    pub fn caret_x(&self, column: CharIndex, prefer_next_row: bool) -> f32 {
        if !prefer_next_row
            && self.glyphs.iter().any(|glyph| glyph.is_rtl)
            && let Some(previous) = column.0.checked_sub(1).and_then(|i| self.glyphs.get(i))
        {
            return if previous.is_rtl {
                previous.pos.x
            } else {
                previous.max_x()
            };
        }
        self.x_offset(column)
    }

    /// Return the logical boundary and affinity nearest the pointer's glyph half.
    pub fn nearest_caret(&self, x: f32) -> (CharIndex, bool) {
        if !self.glyphs.iter().any(|glyph| glyph.is_rtl) {
            let column = self.char_at(x);
            return (column, column < self.char_count_excluding_newline());
        }
        self.glyphs
            .iter()
            .enumerate()
            .map(|(i, glyph)| {
                let leading = (x < glyph.pos.x + glyph.advance_width / 2.0) != glyph.is_rtl;
                let edge = if leading != glyph.is_rtl {
                    glyph.pos.x
                } else {
                    glyph.max_x()
                };
                let distance = (glyph.pos.x - x).max(x - glyph.max_x()).max(0.0);
                (
                    CharIndex(i + usize::from(!leading)),
                    leading,
                    distance,
                    (edge - x).abs(),
                )
            ...
            .map_or((CharIndex::ZERO, true), |(column, leading, _, _)| {
                (column, leading)
            })
    }
...
impl Galley {
    ...
    fn end_pos(&self) -> Rect {
        if let Some(row) = self.rows.last() {
            let x = row.pos.x + row.x_offset(row.char_count_excluding_newline());
    ...
    pub fn pos_from_cursor(&self, cursor: CCursor) -> Rect {
        ...
        let x = row.pos.x + row.caret_x(layout.column, cursor.prefer_next_row);
        Rect::from_min_max(pos2(x, row.min_y()), pos2(x, row.max_y()))
    }
    ...
    pub fn cursor_from_pos(&self, pos: Vec2) -> CCursor {
        ...
        for row in &self.rows {
            ...
            if is_pos_within_row || y_dist < best_y_dist {
                ...
                let (column, prefer_next_row) = row.nearest_caret(pos.x - row.pos.x);
                cursor = CCursor {
...
impl Galley {
    pub fn cursor_left_one_character(&self, cursor: &CCursor) -> CCursor {
        if let Some(next) = self.visual_cursor_step(cursor, false) {
            return next;
        }
    ...
    pub fn cursor_right_one_character(&self, cursor: &CCursor) -> CCursor {
        if let Some(next) = self.visual_cursor_step(cursor, true) {
            return next;
        }
        CCursor {
    ...
    fn visual_cursor_step(&self, cursor: &CCursor, right: bool) -> Option<CCursor> {
        let layout = self.layout_from_cursor(*cursor);
        let row = self.rows.get(layout.row)?;
        if !row.glyphs.iter().any(|g| g.is_rtl) {
            return None;
        }
        let x = row.caret_x(layout.column, cursor.prefer_next_row);
        let next_x = row
            .glyphs
            .iter()
            .flat_map(|glyph| [glyph.pos.x, glyph.max_x()])
            .filter(|&edge| {
                if right {
                    edge > x + 0.01
                } else {
        ...
        if let Some(next_x) = next_x {
            // Approach the boundary from the glyph that was just traversed.
            let pointer_x = if right {
                next_x.next_down()
            } else {
                next_x.next_up()
            };
            return Some(self.cursor_at_row_x(layout.row, pointer_x));
        }
        // At the visual edge, continue on the neighbouring line in reading order.
        let rtl = row.glyphs.first().is_some_and(|g| g.is_rtl);
        let next = if right != rtl {
            layout.row.checked_add(1)
        } else {
            layout.row.checked_sub(1)
        };
        if let Some(next) = next.filter(|&i| i < self.rows.len()) {
            let target = &self.rows[next];
            let edge = target
                .glyphs
                .iter()
                .map(|glyph| if right { glyph.pos.x } else { glyph.max_x() })
            ...
            return Some(self.cursor_at_row_x(next, pointer_x));
        }
        Some(*cursor)
    }

    fn cursor_at_row_x(&self, row: usize, x: f32) -> CCursor {
        let (column, prefer_next_row) = self.rows[row].nearest_caret(x);
        let mut cursor = self.cursor_from_layout(LayoutCursor { row, column });
        cursor.prefer_next_row = prefer_next_row;
        cursor
    }

    pub fn clamp_cursor(&self, cursor: &CCursor) -> CCursor {
        let mut clamped = self.cursor_from_layout(self.layout_from_cursor(*cursor));
        if clamped.index == cursor.index {
            clamped.prefer_next_row = cursor.prefer_next_row;
        }
        clamped
    }
    ...
    pub fn cursor_up_one_row(
        ...
        if layout_cursor.row == 0 {
            ...
            (
                self.cursor_at_row_x(new_row, h_pos - self.rows[new_row].pos.x),
                Some(h_pos),
            )
        }
    }
