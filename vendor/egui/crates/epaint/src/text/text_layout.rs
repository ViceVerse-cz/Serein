...
mod bidi;
use bidi::{layout_rtl_run, reorder_rows, visual_glyphs};
...
pub(crate) fn layout(fonts: &mut FontsImpl, pixels_per_point: f32, job: Arc<LayoutJob>) -> Galley {
    ...
    let bidi = (!job.text.is_ascii()).then(|| unicode_bidi::BidiInfo::new(&job.text, None));
    let bidi = bidi.as_ref().filter(|bidi| bidi.has_rtl());
    ...
    debug_assert!(
        rows.iter()
            .all(|row| row.glyphs.first().is_none_or(|glyph| glyph.cluster_start)),
        "row break split a shaped cluster"
    );
...
fn layout_shaped_run(
    ...
    for (info, pos) in iter::zip(glyph_buffer.glyph_infos(), glyph_buffer.glyph_positions()) {
        ...
        let mut glyph = if glyph_id == skrifa::GlyphId::NOTDEF {
        ...
        glyph.cluster_start = is_new_cluster;
...
fn layout_section(
    ...
    bidi: Option<&unicode_bidi::BidiInfo<'_>>,
    ...
    let mut segment_offset = byte_range.start.0;
    ...
    for (seg_idx, segment) in SplitOrWhole::new(section_text, job.break_on_newline).enumerate() {
        ...
        let levels = bidi.map(|bidi| &bidi.levels[segment_offset..segment_offset + segment.len()]);
        segment_offset += segment.len() + usize::from(job.break_on_newline);
        if segment.is_empty() {
            continue;
        }

        segment_into_runs(fonts, &ctx, segment, levels, &mut runs);
...
fn add_row_backgrounds(point_scale: PointScale, job: &LayoutJob, row: &Row, mesh: &mut Mesh) {
    ...
    for glyph in visual_glyphs(row) {
        let format = &job.sections[glyph.section_index as usize].format;
...
fn tessellate_glyphs(point_scale: PointScale, job: &LayoutJob, row: &mut Row, mesh: &mut Mesh) {
    for glyph in &mut row.glyphs {
        ...
        for uv_rect in iter::once(glyph.uv_rect).chain(
            glyph
                .extra_uv_rects
                .iter()
...
impl RowBreakCandidates {
    fn add(&mut self, index: usize, glyphs: &[Glyph]) {
        if glyphs.get(1).is_some_and(|next| !next.cluster_start) {
            return;
        }
...
///
/// Directional runs are split before shaping, including when scripts share a font.
///
...
fn segment_into_runs(
    ...
    for (byte_offset, grapheme_str) in text.grapheme_indices(true) {
        let level = levels.map_or(unicode_bidi::Level::ltr(), |levels| levels[byte_offset]);
