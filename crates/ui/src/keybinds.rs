...
use model::{KeybindAction, Keybinds, KeyChord};
...
pub(super) fn show(
	...
	ui.label("Make Serein feel like yours. Click any shortcut, then press the key combination you want.");
	design::card(ui, |ui| {
		ui.horizontal(|ui| {
			ui.label(RichText::new("⌨").size(20.0).color(colors.accent));
			ui.vertical(|ui| {
				ui.label(design::semibold(ui, "Shortcuts are saved on this device", 14.0));
				ui.weak("Application shortcuts work while Serein is focused. Push to Talk can also work globally.");
			});
		});
	});

	ui.add_space(18.0);
	section(ui, "Navigation", "Move around Serein without reaching for the mouse.", NAVIGATION, bindings, capturing);
	section(ui, "Messages", "Composer shortcuts are only active while you are writing.", MESSAGES, bindings, capturing);
	section(ui, "Text Formatting", "Apply or remove formatting in the composer.", FORMATTING, bindings, capturing);
	section(ui, "Voice", "Hold the key during a connected call to speak.", &[KeybindAction::PushToTalk], bindings, capturing);
	ui.add_space(10.0);
	ui.label(RichText::new("GLOBAL AVAILABILITY").size(11.0).color(colors.muted));
	...
	if let Some(action) = *capturing {
		...
		if cancelled {
			*capturing = None;
			...
			if let Some(other) = KeybindAction::ALL.into_iter().find(|other| {
				*other != action && bindings.chord(*other) == &chord
			}) {
				ui.colored_label(colors.danger, format!("That shortcut is already used by {}.", other.label()));
			} else {
...
fn modifier_bits(modifiers: Modifiers) -> u8 {
	let mut bits = 0;
	if modifiers.command { bits |= model::keybinds::PRIMARY; }
	if modifiers.shift { bits |= model::keybinds::SHIFT; }
	if modifiers.alt { bits |= model::keybinds::ALT; }
	if modifiers.ctrl && !modifiers.command { bits |= model::keybinds::CTRL; }
	bits
}
...
pub(crate) fn pressed(input: &mut InputState, chord: &KeyChord) -> bool {
	key_name_to_egui(&chord.key).is_some_and(|key| input.consume_key(egui_modifiers(chord.modifiers), key))
}

pub(crate) fn pressed_exact(input: &mut InputState, chord: &KeyChord) -> bool {
	let Some(key) = key_name_to_egui(&chord.key) else { return false };
...
pub(crate) fn down(input: &InputState, chord: &KeyChord) -> bool {
	key_name_to_egui(&chord.key).is_some_and(|key| {
		input.key_down(key) && input.modifiers.matches_logically(egui_modifiers(chord.modifiers))
	})
}
...
fn chord_parts(chord: &KeyChord) -> Vec<String> {
	...
	if chord.modifiers & model::keybinds::PRIMARY != 0 {
		parts.push(if cfg!(target_os = "macos") { "⌘" } else { "Ctrl" }.into());
	}
	if chord.modifiers & model::keybinds::CTRL != 0 {
		parts.push("Ctrl".into());
	}
	if chord.modifiers & model::keybinds::ALT != 0 {
		parts.push(if cfg!(target_os = "macos") { "⌥" } else { "Alt" }.into());
	}
...
fn shortcut_button(ui: &mut egui::Ui, chord: &KeyChord, active: bool) -> egui::Response {
	...
	let widths: Vec<f32> = labels
		.iter()
		.map(|label| ui.painter().layout_no_wrap(label.clone(), font.clone(), colors.text_strong).size().x + 14.0)
		.collect();
	...
	for (label, width) in labels.iter().zip(widths) {
		...
		ui.painter().rect_filled(
			egui::Rect::from_min_max(key_rect.left_top(), key_rect.right_bottom() + egui::vec2(0.0, 2.0)),
			4,
			colors.border,
		);
		...
		let galley = ui.painter().layout_no_wrap(label.clone(), font.clone(), colors.text_strong);
		ui.painter().galley(face.center() - galley.size() * 0.5, galley, colors.text_strong);
		x += width + 4.0;
	}
	response
}
...
fn display_key(name: &str) -> String {
	match name {
		...
		"Enter" => "↵".into(),
		"Slash" => "/".into(),
		name if name.strip_prefix("Num").is_some_and(|digit| digit.len() == 1) => {
			name[3..].into()
		}
...
fn key_name(key: Key) -> Option<&'static str> {
	KEYS.iter().find(|(candidate, _)| *candidate == key).map(|(_, name)| *name)
}

fn key_name_to_egui(name: &str) -> Option<Key> {
	KEYS.iter().find(|(_, candidate)| *candidate == name).map(|(key, _)| *key)
}

const KEYS: &[(Key, &str)] = &[
	(Key::ArrowDown, "ArrowDown"), (Key::ArrowLeft, "ArrowLeft"), (Key::ArrowRight, "ArrowRight"), (Key::ArrowUp, "ArrowUp"),
	(Key::Escape, "Escape"), (Key::Tab, "Tab"), (Key::Backspace, "Backspace"), (Key::Enter, "Enter"), (Key::Space, "Space"),
	(Key::Delete, "Delete"), (Key::Home, "Home"), (Key::End, "End"), (Key::Slash, "Slash"), (Key::Backtick, "Backtick"), (Key::Minus, "Minus"), (Key::Equals, "Equals"), (Key::Comma, "Comma"), (Key::Period, "Period"),
	(Key::Num0, "Num0"), (Key::Num1, "Num1"), (Key::Num2, "Num2"), (Key::Num3, "Num3"), (Key::Num4, "Num4"), (Key::Num5, "Num5"), (Key::Num6, "Num6"), (Key::Num7, "Num7"), (Key::Num8, "Num8"), (Key::Num9, "Num9"),
	(Key::A, "A"), (Key::B, "B"), (Key::C, "C"), (Key::D, "D"), (Key::E, "E"), (Key::F, "F"), (Key::G, "G"), (Key::H, "H"), (Key::I, "I"), (Key::J, "J"), (Key::K, "K"), (Key::L, "L"), (Key::M, "M"), (Key::N, "N"), (Key::O, "O"), (Key::P, "P"), (Key::Q, "Q"), (Key::R, "R"), (Key::S, "S"), (Key::T, "T"), (Key::U, "U"), (Key::V, "V"), (Key::W, "W"), (Key::X, "X"), (Key::Y, "Y"), (Key::Z, "Z"),
...
mod tests {
	...
	fn default_bindings_round_trip_and_match() {
		...
		assert_eq!(chord_parts(bindings.chord(KeybindAction::SwitchConversation)).join(" + "), "Ctrl + K");
		let ctx = egui::Context::default();
		let mut matched = false;
		ctx.run_ui(
			egui::RawInput { events: vec![Event::Key { key: Key::V, physical_key: None, pressed: true, repeat: false, modifiers: Modifiers::NONE }], ..Default::default() },
			|ui| matched = ui.input_mut(|input| pressed(input, bindings.chord(KeybindAction::PushToTalk))),
		);
		assert!(matched);
	}
}
