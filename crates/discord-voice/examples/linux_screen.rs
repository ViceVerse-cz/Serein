...
#[cfg(unix)]
use screen::{
	EncodedFrame, MAX_ENCODED_BYTES, MAX_RAW_BYTES, RawFrame, Settings, SourceId, encode_pixels,
	encoder, preview_frame,
};
#[cfg(unix)]
...
fn main() {
	...
	runtime.block_on(async {
		...
		drop(pipeline);
		println!("Linux screen pipeline: synthetic preview, secure-readiness gate, software H.264 and portal pre-cancellation passed. Native Linux capture/GPU encoding remains unverified.");
	});
}
#[cfg(not(unix))]
fn main() {
